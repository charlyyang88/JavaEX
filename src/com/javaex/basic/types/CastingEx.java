...
public class CastingEx {
	...
	public static void castingEx() {
		// 표현 번위가 넓은 자료형 -> 좁은 자료형으로의 변환
		// 데이터의 유실 위험 -> 반드시 강제 타입 변환이 필요
		double d = 1234567.890;
		System.out.println(d);
		
		float f = (float)d;
		System.out.println(f);	// 8byte 실수 -> 4byte 실수
		
		long l = (long)f;	// 4byte 실수 -> 8byte 정수
		System.out.println(l);
		
		short s = (short)l;	// 8byte 정수 -> 2byte 정수
	...
	public static void promotionEx() {
		...
		double d = f;	// 4byte 실수 -> 8byte 상수
		System.out.println(d);
		
		char ch = 'A';
		short s2 = (short)ch;	// unsigned int (2) -> 2byte 정수
